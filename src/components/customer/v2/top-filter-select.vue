		...
		<div class="filter-option-main" v-show="togglevalue">
			<ul>
				<li>
					...
					<Radio-group v-model="filterData.partner_type" type="button">
						<Radio label="0"><span>普通客户</span></Radio>
						<Radio label="1"><span>项目客户</span></Radio>
					</Radio-group>
				...
				<li>
					<h3>下单方式</h3>
					<Radio-group v-model="filterData.partner_order_form" type="button">
		...
		watch: {
			filterData:{
				handler: function(curVal,oldVal){
					...
					let num = 0;
					this.filterData.partner_type!=''?num++:'';
					this.filterData.invite_type!=''?num++:'';
					this.filterData.partner_order_form!=''?num++:'';
					this.count=num
    　　　　　　　},
                deep:true
			}
		...
		methods: {
			...
			clearCondion(){
				this.filterData.partner_type=''
				this.filterData.invite_type=''
				this.filterData.partner_order_form=''
			},
			handleClose(){
