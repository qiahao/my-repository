<template>
	<div>
        <div class="">
            <span class="subtitle">
                <Icon type="android-contacts" class="subtitle-icon"></Icon>部门人员
            </span>
            <i-button  type="primary" size="small" @click='addStaff'>新增员工</i-button>
        </div>
        <div class="row-table">
...
<script>
	...
	export default {
		...
		data (){
			return {
                ...
                columns: [
                    ...
                    {title: '帐号',key: 'user_mobile', width : 120}, 
                    {title: '手机号',key: 'user_mobile', width : 120}, 
                    {title: '角色',key: 'role_name'}, 
                    {title: '状态',key: 'user_status', width : 100, render: (h, params) => {
                            let st = {Y:"启用", N:"禁用"}
                            return h('span',st[params.row.user_status])
                        }
                    },
                    { title: ' ', key: 'user_status', width : 250, render: (h,params)=>{
                        return h('div',[
                                h('a', {
                                    on:{
		...
		methods: {
            reloadList(deptId, deptName) {
                this.deptModal.dept_id = deptId;
                this.deptModal.dept_name = deptName;
                this.loadList();
            },
            loadList (){
                let params = {act_user_id: this.userInfo.user_id, dept_id: this.deptModal.dept_id};
                API.getSelectedUserByDeptId(params).then((data)=>{
                    this.tableParams.list = data.resultObj
                    this.tableParams.total = this.tableParams.list.length;
                })
            },
            //新增员工
            addStaff (){
                ...
                this.addModal.modalData.dept_id = this.deptModal.dept_id;
                this.$refs.deptDropDownTree.setSelected(this.deptModal.dept_id, this.deptModal.dept_name);
                this.$refs.roleDropDownTree.setSelected('', '');
            ...
            editStaff (staff){
                ...
                /*this.$refs.editDeptDropDownTree.loadDeptList(staff.user_id);
                this.$refs.editRoleDropDownTree.loadRoleList(staff.user_id);*/
            },
            saveEditStaff (){
