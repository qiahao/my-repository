<template>
    <div>
        <Modal v-model="modal" title="异常登记" @on-cancel="doCancel">
            <i-form ref="exceptionForm" :label-width="100" :model="exceptionInfo" :rules="exceptionRule">
                <Form-item label="异常信息：" prop="ex_content">
                    <i-input type="text" v-model="exceptionInfo.ex_content"></i-input>
                </Form-item>
                ...
                <Form-item label="处理情况：" prop="ex_result">
                    <i-input type="textarea" v-model="exceptionInfo.ex_result"></i-input>
                </Form-item>
            </i-form>
            <div slot="footer">
                <i-button type="primary" size="large" @click="doOk">确认</i-button>
        ...
        doOk() {
            this.exceptionForm.validate((valid) => {
                if (valid) {
                    ...
                    apis.addException(params).then((data) => {
                        this.$Message.success({ content: data.resultInfo, duration: 1 })
                        this.modal = false
                        bus.$emit('order-expeption-reload')
                        bus.$emit('operation-reload')
                        bus.$emit('waybill-list-reload')
                    })
                }
            })
